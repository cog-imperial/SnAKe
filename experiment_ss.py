...
if merge_method == 1:
    merge_method = 'e-Point Deletion'
elif merge_method == 2:
    merge_method = 'Self-stopping'
...
hypers = (0.7, torch.tensor([0.2, 0.2, 0.2]), 0.01, 0)
...
if method == 'SnAKe':
    mod = SnAKe(env, merge_method = merge_method, merge_constant = epsilon, cost_function = cost_function, initial_temp = initial_temp, \
        hp_update_frequency = 25, delta_std = delta_std)
elif method == 'EI':
...
if method == 'SnAKe':
    folder_inputs = 'experiment_results_ss/' + f'{epsilon}-EaS/' + func.name + f'/budget{budget + 1}/' + cost_name + '/inputs/'
    folder_outputs = 'experiment_results_ss/' + f'{epsilon}-EaS/' + func.name + f'/budget{budget + 1}/' + cost_name + '/outputs/'
    file_name = f'run_{run_num}'
elif method == 'Random':
    folder_inputs = 'experiment_results_ss/' + f'Random/' + func.name + f'/budget{budget + 1}/' + cost_name + '/inputs/'
    folder_outputs = 'experiment_results_ss/' + f'Random/' + func.name + f'/budget{budget + 1}/' + cost_name + '/outputs/'
    file_name = f'run_{run_num}'
elif method == 'EIpu':
    folder_inputs =  'experiment_results_ss/' + f'{epsilon}-' + str(gamma) + method + '/' + func.name + '/' + f'/budget{budget + 1}/inputs/'
    folder_outputs =  'experiment_results_ss/' + + f'{epsilon}-' + str(gamma) + method + '/' + func.name + '/' + f'/budget{budget + 1}/outputs/'
    file_name = f'run_{run_num}'
else:
    folder_inputs =  'experiment_results_ss/' + method + '/' + func.name + '/' + f'/budget{budget + 1}/inputs/'
    folder_outputs =  'experiment_results_ss/' + method + '/' + func.name + '/' + f'/budget{budget + 1}/outputs/'
    file_name = f'run_{run_num}'
...
print(np.log(func.optimum - max(Y)))
